...
public class NewServiceWizardIterator implements WizardDescriptor.InstantiatingIterator {
    ...
    public Set instantiate() throws IOException {
        ...
        String gwtOutputDir = GWTProjectInfo.getGWTOutputDir(project);

        if (!gwtOutputDir.endsWith("/")){
            gwtOutputDir = gwtOutputDir + "/";
        }
        ...
        StringBuilder clientSourcesRoot = new StringBuilder(GWTProjectInfo.getClientPackage(project));

        if (subpackage != null && subpackage.length() > 0){
            clientSourcesRoot.append("." + subpackage);
        }
        
        Util.replaceInFile(doServiceImpl.getPrimaryFile(),
                new String[]{"__CLIENT_SOURCES_ROOT__", "__SERVICE_NAME__"},
                new String[]{clientSourcesRoot.toString(), serviceName});
        ...
        if ((Boolean)wizard.getProperty(CREATE_USAGE_EXAMPLE)){
            ...
            Util.replaceInFile(doExample.getPrimaryFile(),
                ...
                new String[]{servletMapping, serviceName});
