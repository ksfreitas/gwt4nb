...
public class JavaModelUtils {
    ...
    public static ExecutableElement[] getMethod(TypeElement clazz,
            String methodName) {
        List<ExecutableElement> methods = new ArrayList<ExecutableElement>();

        for (ExecutableElement method : ElementFilter.methodsIn(clazz.
                getEnclosedElements())) {
     ...
     */
    public static boolean isMethodSignatureEqual(ExecutableElement syncMethod,
            ExecutableElement[] asyncMethods) {
        boolean matchFound = false;
        for (ExecutableElement exeEle: asyncMethods) {
            List<? extends VariableElement> syncMethodParamsList = syncMethod.
                    getParameters();
            List<? extends VariableElement> asyncMethodParamsList = exeEle.
                    getParameters();
            VariableElement[] asyncMethodParamArray =
                    asyncMethodParamsList.toArray(
                    new VariableElement[asyncMethodParamsList.size()]);

            // if number of parameters don't match or AsyncCallback parameter
            // is missing, skip the method
            if (syncMethodParamsList.size() !=
                    (asyncMethodParamsList.size() - 1)) {
                continue;
            }

            /*
            if (!(exeEle.getReturnType() instanceof NoType))
                continue;*/
            ...
            Logger.getAnonymousLogger().severe(lastParamType + "--------");

            // startsWith because of generic class
            if (!lastParamType.startsWith(
            ...
            for (VariableElement syncMethodParam : syncMethodParamsList) {
                ...
                if (!syncMethodParam.asType().toString().equals(asyncMethodParamArray[syncParamIndex].
                        asType().toString())) {
                    break;
                }
    ...
    public static boolean isMathcingServiceMethodFound(
            ExecutableElement asyncMethod, ExecutableElement[] syncMethods) {
        boolean matchFound = false;
        List<? extends VariableElement> asyncMethodParamsList = asyncMethod.
                getParameters();
        VariableElement[] asyncMethodParamArray =
                asyncMethodParamsList.toArray(
                new VariableElement[asyncMethodParamsList.size()]);
        ...
        // startsWith is used for generic types
        if (!lastParamType.startsWith("com.google.gwt.user.client.rpc.AsyncCallback")) {
            return false;
        }

        for (ExecutableElement exeEle : syncMethods) {
            List<? extends VariableElement> syncMethodParamsList = exeEle.
                    getParameters();
     ...
     */
    public static List<ElementHandle> getUnmatchedMethods(TypeElement clazzA,
            TypeElement clazzB) {
        List<ElementHandle> unmatchedMethods = new ArrayList<ElementHandle>();
        for (ExecutableElement aMethod : ElementFilter.methodsIn(clazzA.
                getEnclosedElements())) {
    ...
    public static String extractClassNameFromType(TypeMirror type) {
        if (type instanceof DeclaredType) {
            ...
            if (elem.getKind() == ElementKind.CLASS || elem.getKind() ==
    ...
    public static String getShortClassName(String qualifiedClassName) {
        return qualifiedClassName.substring(qualifiedClassName.lastIndexOf(".") +
                1); //NOI18N
    }
}
